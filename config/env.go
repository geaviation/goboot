//The MIT License (MIT) Copyright (c) 2015-2016 Carlos Alexandro Becker

// Code is taken from github.com/caarlos0/env and modified to handle JSON values
// for examples, please visit the github repo
package config
...
func osGetenv(key string) string {
	p := strings.Split(key, ".")
	if len(p) == 1 {
		return settings.GetStringEnv(p[0])
	}
